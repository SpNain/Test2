function addPaginationNav(
  ...
  if (status)
    prevLi.addEventListener("click", () => {
      if (currentPage > 1) passedFunction(currentPage - 1, status);
    });
  else
    prevLi.addEventListener("click", () => {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      i === currentPage
        ? a.setAttribute("class", "page-link activePage")
        : null;

      a.appendChild(document.createTextNode(i));
      li.appendChild(a);
      ul.appendChild(li);
      if (status) a.addEventListener("click", () => passedFunction(i, status));
      else a.addEventListener("click", () => passedFunction(i));
    }
  // Next Button
  ...
  nextLi.addEventListener("click", () => {
    if (currentPage < totalPages) passedFunction(currentPage + 1, status);
  });
  else
  nextLi.addEventListener("click", () => {
